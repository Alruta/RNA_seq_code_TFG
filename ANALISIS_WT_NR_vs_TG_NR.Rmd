...
# GRAFICAS COMPOSICIÓN DE LIBRERIAS HECHAS EN OTROS SCRIPTS
...
# COMPARACIONES DEL MISMO GRUPO HECHAS. COMPARO ENTRE GRUPOS.

png("Comparación entre muestras WT-NR y TG-NR 1.png", width = 400, height = 300)
plot(logcountsdata$WT_NR_1, logcountsdata$TG_NR_1, xlab = "Composición TG-NR 1", ylab = "Composición WT-NR 1", main = "Comparación entre muestras WT-NR y TG-NR 1", col = "brown")
dev.off()

png("Comparación entre muestras WT-NR y TG-NR 2.png", width = 400, height = 300)
plot(logcountsdata$WT_NR_2, logcountsdata$TG_NR_2, xlab = "Composición TG-NR 2", ylab = "Composición WT-NR 2", main = "Comparación entre muestras WT-NR y TG-NR 2", col = "brown")
dev.off()

png("Comparación entre muestras WT-NR y TG-NR 3.png", width = 400, height = 300)
plot(logcountsdata$WT_NR_3, logcountsdata$TG_NR_3, xlab = "Composición TG-NR 3", ylab = "Composición WT-NR 3", main = "Comparación entre muestras WT-NR y TG-NR 3", col = "brown")
dev.off()

png("Comparación entre muestras WT-NR y TG-NR 4.png", width = 400, height = 300)
plot(logcountsdata$WT_NR_4, logcountsdata$TG_NR_4, xlab = "Composición TG-NR 4", ylab = "Composición WT-NR 4", main = "Comparación entre muestras WT-NR y TG-NR 4", col = "brown")
dev.off()
...
#write.table(norm_Counts, file="Normalized_Counts_WT-NR_vs_TG-NR.txt", sep="\t", quote = FALSE, row.names = TRUE) 
...
res <- results(dds, list( c( "Genotipo_TG_vs_WT","GenotipoTG.TratamientoNR")))
# Extraer resultados de dds para comparar WT_NR vs TG_NR. HAY DIFERENCIAS ENTRE WT Y TG CON NR
...
### 3.2. MA plot 
```{r MA plot}
png("MA plot 1 (WT-NR vs TG-NR).png", width = 500, height = 400)
DESeq2::plotMA(res, ylim=c(-25,25), xlab = "Media normalizada de lecturas", ylab = "Log2 Fold Change", main = "Gráfico MA WT-NR vs TG-NR", colSig = "orange", colNonSig = "darkgray")
...
png("MA plot 2 (WT-NR vs TG-NR).png", width = 500, height = 400)
myColors <- c("darkgray", "orange")
...
MA_plot <- MA_plot + colScale + labs(title="Gráfico MA WT-NR vs TG-NR") + theme (plot.title = element_text(hjust = 0.5))  
...
png("Vulcano plot (WT-NR vs TG-NR).png", width = 500, height = 400)

Vulcano <- ggplot(res_Ordered_DF, main = "Gráfica en volcan WT-NR vs TG-NR", aes(x = log2FoldChange, y = -log10(padj) , color = GENES)) + geom_point() + theme(legend.position = "bottom") 

Vulcano <- Vulcano + colScale + labs(title= "Gráfica en volcan WT-NR vs TG-NR") + theme (plot.title = element_text(hjust = 0.5)) 
...
FORMA 1 con rlog
```{r Gráfico de principales componentes HECHO EN WT-CONTROL VS WT-NR}
...
Sig_counts <- All_sig[,c("WT_NR_1", "WT_NR_2", "WT_NR_3", "WT_NR_4", "TG_NR_1", "TG_NR_2", "TG_NR_3", "TG_NR_4" )]
...
png("Heatmap WT-NR vs TG-NR.png", width = 600, height = 500)
pheatmap(log2(Sig_counts + 1), scale = 'row', show_rownames = FALSE, treeheight_row = FALSE, treeheight_col = FALSE, main = "Mapa de calor entre WT-NR y TG-NR", labels_col = c("WT-NR 1", "WT-NR 2", "WT-NR 3", "WT-NR 4", "TG-NR 1", "TG-NR 2", "TG-NR 3", "TG-NR 4"), angle_col = 45, cluster_cols = F, border_color = FALSE)
dev.off()
```
...
Plot_counts <- plotCounts(dds[,c("WT_NR_1", "WT_NR_2", "WT_NR_3", "WT_NR_4", "TG_NR_1", "TG_NR_2", "TG_NR_3", "TG_NR_4" )], gene=topGene, intgroup = c("Genotipo", "Tratamiento"), returnData = TRUE) 
...
png("TOP_gene WT-NR vs TG-NR.png", width = 600, height = 500)
Plot_counts <- Plot_counts + labs(title="Expresión diferencial de Ppp1r9b", x = "Grupo", y = "Counts") + theme (plot.title = element_text(hjust = 0.5)) 
...
keep <- c("WT_NR_1", "WT_NR_2", "WT_NR_3", "WT_NR_4", "TG_NR_1", "TG_NR_2", "TG_NR_3", "TG_NR_4" )

Top_10 <- Top_10[,names(Top_10) %in% keep]

png("TOP_10 heatmap WT-NR vs TG-NR.png", width = 600, height = 500)

pheatmap(log2(Top_10 +1), treeheight_row = FALSE, treeheight_col = FALSE, main = "Mapa de calor con los 10 principales genes entre WT-NR y TG-NR", labels_col = c("WT-NR 1", "WT-NR 2", "WT-NR 3", "WT-NR 4", "TG-NR 1", "TG-NR 2", "TG-NR 3", "TG-NR 4"), angle_col = 45, cluster_cols = F, border_color = FALSE)
...
png("TOP_10 WT-NR vs TG-NR.png", width = 600, height = 500)

TOP_GENES <- ggplot(Top_10_m , aes( x = Genotipo, y = log2(Exp +1), color = Genotipo)) + geom_point() + facet_grid(~ genes) + labs(title="Expresión diferencial de los principales genes entre WT-NR vs TG-NR", x = "", y = "Log2 de las lecturas") + theme (plot.title = element_text(hjust = 0.5), legend.position = "bottom", axis.text.x =element_blank()) + colScale2
...
write.table(DE_GENES, "DE_GENES_WT_NR_vs_TG_NR.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
write.table(GENES_UP, "GENES_UP_WT_NR_vs_TG_NR.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(GENES_DOWN, "GENES_DOWN_WT_NR_vs_TG_NR.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
write.table(Background_Genes, "Background_Genes_WT_NR_vs_TG_NR.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
