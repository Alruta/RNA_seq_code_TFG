...
```{r Representación de los counts por muestra. Común y Correcto en WT-control vs WT-NR}
...
# Composición librerias WT-control en script WT-control vs WT-NR

png("Libreria  TG-control 1.png", width = 400, height = 300)
hist(logcountsdata$TG_control_1, br=20, xlab = "Logaritmo del número de lecturas", ylab = "Frecuencia", main = "Composición de la librería del TG-control 1", col = "darkolivegreen2", ylim = c(0,80000))
dev.off()

png("Libreria  TG-control 2.png", width = 400, height = 300)
hist(logcountsdata$TG_control_2, br=20, xlab = "Logaritmo del número de lecturas", ylab = "Frecuencia", main = "Composición de la librería del TG-control 2", col = "darkolivegreen2", ylim = c(0,80000))
dev.off()

png("Libreria  TG-control 3.png", width = 400, height = 300)
hist(logcountsdata$TG_control_3, br=20, xlab = "Logaritmo del número de lecturas", ylab = "Frecuencia", main = "Composición de la librería del TG-control 3", col = "darkolivegreen2", ylim = c(0,80000))
dev.off()
...
# Comparaciones WT-control entre sí en script WT-control vs WT-NR

png("Comparación entre muestras TG-control 1.png", width = 400, height = 300)
plot(logcountsdata$TG_control_1, logcountsdata$TG_control_2, xlab = "Composición TG-control 2", ylab = "Composición TG-control 1", main = "Comparación entre muestras TG-control 1", col = "Darkblue")
dev.off()

png("Comparación entre muestras TG-control 2.png", width = 400, height = 300)
plot(logcountsdata$TG_control_1, logcountsdata$TG_control_3, xlab = "Composición TG-control 3", ylab = "Composición TG-control 1", main = "Comparación entre muestras TG-control 2", col = "Darkblue")
dev.off()

png("Comparación entre muestras WT-control y TG-control 1.png", width = 400, height = 300)
plot(logcountsdata$TG_control_1, logcountsdata$WT_control_1, xlab = "Composición WT-control 1", ylab = "Composición TG-control 1", main = "Comparación entre muestras WT-control y TG-control 1", col = "green4")
dev.off()

png("Comparación entre muestras WT-control y TG-control 2.png", width = 400, height = 300)
plot(logcountsdata$TG_control_3, logcountsdata$WT_control_3, xlab = "Composición WT-control 3", ylab = "Composición TG-control 3", main = "Comparación entre muestras WT-control y TG-control 2", col = "green4")
dev.off()
...
#write.table(norm_Counts, file="Normalized_Counts_WT_control_vs_TG_control.txt", sep="\t", quote = FALSE, row.names = TRUE) 
...
res <- results(dds, contrast = c("Genotipo", "TG", "WT"))
# Extraer resultados de dds para comparar TG_control vs WT_control. EFECTO DE LA SOBREEXPRESIÓN
...
# write.table(res_Ordered, file="res_Ordered.txt", sep="\t", quote = FALSE, row.names = TRUE) 
# Archivo que pasar a excel pasando puntos a comas excepto en los nombres de los genes
...
write.table(res_Ordered_DF, file = "RESULTADOS_WT-control_vs_TG-control.txt", sep = "\t", quote = FALSE)
```

### 3.2. MA plot 
```{r MA plot}

png("MA plot 1 (WT-controlvsTG-control).png", width = 500, height = 400)
DESeq2::plotMA(res, ylim=c(-25,25), xlab = "Media normalizada de lecturas", ylab = "Log2 Fold Change", main = "Gráfico MA WT-control vs TG-control", colSig = "red", colNonSig = "darkgray")
...
png("MA plot 2 (WT-controlvsTG-control).png", width = 500, height = 400)
myColors <- c("darkgray", "red")
...
MA_plot <- MA_plot + colScale + labs(title="Gráfico MA WT-control vs TG-control") + theme (plot.title = element_text(hjust = 0.5))  
...
VERSIÓN EASY
```{r Volcano plot easy}
png("Vulcano plot (WT-control vs TG-control).png", width = 500, height = 500)
...
FORMA 1 con rlog
```{r PCA 1. Comun y correcta en WT-control vs WT-NR}
...
```{r PCA 2. Comun y correcta en WT-control vs WT-NR }
...
```{r Heatmap similitud. Comun y correcto en WT-control vs WT-NR }
...
Sig_counts <- All_sig[,c("WT_control_1", "WT_control_2", "WT_control_3", "WT_control_4", "TG_control_1", "TG_control_2", "TG_control_3")]
...
png("Heatmap WT-control vs TG-control.png", width = 600, height = 500)
pheatmap(log2(Sig_counts + 1), scale = 'row', show_rownames = FALSE, treeheight_row = FALSE, treeheight_col = FALSE, main = "Mapa de calor entre grupos WT-control y TG-control", labels_col = c("WT-control 1", "WT-control 2", "WT-control 3", "WT-control 4", "TG-control 1", "TG-control 2", "TG-control 3"), angle_col = 45, cluster_cols = F, border_color = FALSE)
dev.off()
```
...
Plot_counts <- plotCounts(dds[,c("WT_control_1", "WT_control_2", "WT_control_3", "WT_control_4", "TG_control_1", "TG_control_2", "TG_control_3")], gene=topGene, intgroup = c("Genotipo", "Tratamiento"), returnData = TRUE) 
...
png("TOP_gene WT-control vs TG-control.png", width = 600, height = 500)

Plot_counts <- Plot_counts + labs(title="Expresión diferencial de Mbnl1", x = "Genotipo", y = "Counts") + theme (plot.title = element_text(hjust = 0.5)) 
...
keep <- c("WT_control_1", "WT_control_2", "WT_control_3", "WT_control_4", "TG_control_1", "TG_control_2", "TG_control_3")

Top_10 <- Top_10[,names(Top_10) %in% keep]

png("TOP_10 Heatmap WT-control vs TG-control.png", width = 600, height = 500)

pheatmap(log2(Top_10 +1), treeheight_row = FALSE, treeheight_col = FALSE, main = "Mapa de calor con los 10 principales genes WT-control vs TG-control", labels_col = c("WT-control 1", "WT-control 2", "WT-control 3", "WT-control 4",  "TG-control 1", "TG-control 2", "TG-control 3"), angle_col = 45, cluster_cols = F, border_color = FALSE)
...
myColors2 <- c("darkblue", "red")
names(myColors2) <- levels(Top_10_m$Genotipo)
colScale2 <- scale_colour_manual(name = "Genotipo",values = myColors2)

png("TOP_10 WT-control vs TG-control.png", width = 600, height = 500)

TOP_GENES <- ggplot(Top_10_m , aes( x = Genotipo, y = log2(Exp +1), color = Genotipo)) + geom_point() + facet_grid(~ genes) + labs(title="Expresión diferencial de los principales genes WT-control vs TG-control", x = "", y = "Log2 de las lecturas") + theme (plot.title = element_text(hjust = 0.5), legend.position = "bottom", axis.text.x =element_blank()) + colScale2
...
write.table(DE_GENES, "DE_GENES_WT_control_vs_TG_control.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
write.table(GENES_UP, "GENES_UP_WT_control_vs_TG_control.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(GENES_DOWN, "GENES_DOWN_WT_control_vs_TG_control.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
write.table(Background_Genes, "Background_Genes_WT_control_vs_TG_control.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
