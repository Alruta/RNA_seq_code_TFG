---
title: "ANALISIS: TG-control vs TG-NR"
...
# TG-CONTROL YA HECHO EN WT-CONTROL VS TG-CONTROL
png("Libreria  TG-NR 1.png", width = 400, height = 300)
hist(logcountsdata$TG_NR_1, br=20, xlab = "Logaritmo del número de lecturas", ylab = "Frecuencia", main = "Composición de la librería del TG-NR 1", col = "darkorchid2", ylim = c(0,80000))
dev.off()

png("Libreria  TG-NR 2.png", width = 400, height = 300)
hist(logcountsdata$TG_NR_2, br=20, xlab = "Logaritmo del número de lecturas", ylab = "Frecuencia", main = "Composición de la librería del TG-NR 2", col = "darkorchid2", ylim = c(0,80000))
dev.off()

png("Libreria  TG-NR 3.png", width = 400, height = 300)
hist(logcountsdata$TG_NR_3, br=20, xlab = "Logaritmo del número de lecturas", ylab = "Frecuencia", main = "Composición de la librería del TG-NR 3", col = "darkorchid2", ylim = c(0,80000))
dev.off()

png("Libreria  TG-NR 4.png", width = 400, height = 300)
hist(logcountsdata$TG_NR_4, br=20, xlab = "Logaritmo del número de lecturas", ylab = "Frecuencia", main = "Composición de la librería del TG-NR 4", col = "darkorchid2", ylim = c(0,80000))
dev.off()
...
# COMPARACIONES ENTRE TG-CONTROL HECHAS EN WT-CONTROL VS TG-CONTROL

png("Comparación entre muestras TG-NR 1.png", width = 400, height = 300)
plot(logcountsdata$TG_NR_1, logcountsdata$TG_NR_2, xlab = "Composición TG-NR 2", ylab = "Composición TG-NR 1", main = "Comparación entre muestras TG-NR", col = "orange")
dev.off()

png("Comparación entre muestras TG-NR 2.png", width = 400, height = 300)
plot(logcountsdata$TG_NR_3, logcountsdata$TG_NR_4, xlab = "Composición TG-NR 4", ylab = "Composición TG-NR 3", main = "Comparación entre muestras TG-NR", col = "orange")
dev.off()

png("Comparación entre muestras TG-control y TG-NR 1.png", width = 400, height = 300)
plot(logcountsdata$TG_control_1, logcountsdata$TG_NR_1, xlab = "Composición TG-NR 1", ylab = "Composición TG-control 1", main = "Comparación entre muestras TG-control y TG-NR 1", col = "salmon2")
dev.off()

png("Comparación entre muestras TG-control y TG-NR 2.png", width = 400, height = 300)
plot(logcountsdata$TG_control_2, logcountsdata$TG_NR_2, xlab = "Composición TG-NR 2", ylab = "Composición TG-control 2", main = "Comparación entre muestras TG-control y TG-NR 2", col = "salmon2")
dev.off()

png("Comparación entre muestras TG-control y TG-NR 3.png", width = 400, height = 300)
plot(logcountsdata$TG_control_3, logcountsdata$TG_NR_3, xlab = "Composición TG-NR 3", ylab = "Composición TG-control 3", main = "Comparación entre muestras TG-control y TG-NR 3", col = "salmon2")
dev.off()
...
#write.table(norm_Counts, file="Normalized_Counts_TG_control_vs_TG_NR.txt", sep="\t", quote = FALSE, row.names = TRUE) 
...
res <- results(dds, list( c("Tratamiento_NR_vs_control","GenotipoTG.TratamientoNR")))
# Extraer resultados de dds para comparar TG_NR vs TG_control. EFECTO DEL NR EN LOS TG
...
write.table(res_Ordered_DF, file = "RESULTADOS_TG-control_vs_TG-NR.txt", sep = "\t", quote = FALSE)
```

### 3.2. MA plot 
```{r MA plot}

png("MA plot 1 (TG-control vs TG-NR).png", width = 500, height = 400)
DESeq2::plotMA(res, ylim=c(-25,25), xlab = "Media normalizada de lecturas", ylab = "Log2 Fold Change", main = "Gráfico MA TG-control vs TG-NR", colSig = "green", colNonSig = "darkgray")
...
png("MA plot 2 (TG-control vs TG-NR).png", width = 500, height = 400)
myColors <- c("darkgray", "green")
...
MA_plot <- MA_plot + colScale + labs(title="Gráfico MA TG-control vs TG-NR") + theme (plot.title = element_text(hjust = 0.5))  
...
png("Vulcano plot (TG-control vs TG-NR).png", width = 500, height = 400)
Vulcano <- ggplot(res_Ordered_DF, main = "Gráfico en volcán TG-control vs TG-NR", aes(x = log2FoldChange, y = -log10(padj) , color = GENES)) + geom_point() + theme(legend.position = "bottom") 

Vulcano <- Vulcano + colScale + labs(title="Gráfico en volcán TG-control vs TG-NR") + theme (plot.title = element_text(hjust = 0.5)) 
...
Sig_counts <- All_sig[,c("TG_control_1", "TG_control_2", "TG_control_3", "TG_NR_1", "TG_NR_2", "TG_NR_3", "TG_NR_4")]
...
png("Heatmap TG-control vs TG-NR.png", width = 600, height = 500)
pheatmap(log2(Sig_counts + 1), scale = 'row', show_rownames = FALSE, treeheight_row = FALSE, treeheight_col = FALSE, main = "Mapa de calor entre TG-control y TG-NR", labels_col = c("TG-control 1", "TG-control 2", "TG-control 3", "TG-NR 1", "TG-NR 2", "TG-NR 3", "TG-NR 4"), angle_col = 45, cluster_cols = F, border_color = FALSE)
dev.off()
...
Plot_counts <- plotCounts(dds[,c("TG_control_1", "TG_control_2", "TG_control_3", "TG_NR_1", "TG_NR_2", "TG_NR_3", "TG_NR_4")], gene=topGene, intgroup = c("Genotipo", "Tratamiento"), returnData = TRUE) 
...
png("TOP_gene TG-control vs TG-NR.png", width = 600, height = 500)

Plot_counts <- Plot_counts + labs(title="Expresión diferencial de Eif4g1", x = "Grupo", y = "Counts") + theme (plot.title = element_text(hjust = 0.5)) 
...
keep <- c("TG_control_1", "TG_control_2", "TG_control_3", "TG_NR_1", "TG_NR_2", "TG_NR_3", "TG_NR_4")

Top_10 <- Top_10[,names(Top_10) %in% keep]

png("TOP_10 Heatmap TG-control vs TG-NR.png", width = 600, height = 500)

pheatmap(log2(Top_10 +1), treeheight_row = FALSE, treeheight_col = FALSE, main = "Mapa de calor con los 10 principales genes TG-control vs TG-NR", labels_col = c("TG-control 1", "TG-control 2", "TG-control 3", "TG-NR 1", "TG-NR 2", "TG-NR 3", "TG-NR 4"), angle_col = 45, cluster_cols = FALSE, border_color = FALSE)
dev.off()
...
names(myColors2) <- levels(Top_10_m$Tratamiento)
colScale2 <- scale_colour_manual(name = "Tratamiento",values = myColors2)

png("TOP_10 TG-control vs TG-NR.png", width = 600, height = 500)

TOP_GENES <- ggplot(Top_10_m , aes( x = Tratamiento, y = log2(Exp +1), color = Tratamiento)) + geom_point() + facet_grid(~ genes) + labs(title="Expresión diferencial de los principales genes TG-control vs TG-NR", x = "", y = "Log2 de las lecturas") + theme (plot.title = element_text(hjust = 0.5), legend.position = "bottom", axis.text.x =element_blank()) + colScale2
...
write.table(DE_GENES, "DE_GENES_TG-control_vs_TG-NR.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
write.table(GENES_UP, "GENES_UP_TG-control_vs_TG-NR.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(GENES_DOWN, "GENES_DOWN_TG-control_vs_TG-NR.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
write.table(Background_Genes, "Background_Genes_TG-control_vs_TG-NR.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
